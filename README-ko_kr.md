...
프롬프트 키워드 린트       verify 판정을 실행 증거가 아닌 프롬프트 텍스트 점검으로 명확히 표기
```

자기 점검 블록은 보조 장치다. `prompt_ops_maker.py verify`가 반환하는 `external_verdict.verdict`는 필수 가드레일 문구 존재를 확인한다. 테스트, 빌드, HTTP probe, browser smoke가 실제로 통과했다는 보증은 아니다.

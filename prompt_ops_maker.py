...
def infer_complexity_tier(*, mode: str, effort: str, task: str, config: dict[str, Any]) -> str:
    """Classify prompt-operation complexity for variable L0-L5 control."""
    combined = " ".join(
        [
            mode,
            effort,
            task,
            " ".join(config.get("core_focus", [])),
            " ".join(config.get("verification_gates", [])),
            " ".join(config.get("_extra_risks", [])),
        ]
    ).lower()
    high_markers = [
        "deep",
    ...
    if effort in {"high", "xhigh"} or mode in {"deep-audit", "appsec", "deploy"}:
        return "deep"
    if any(marker in combined for marker in high_markers):
        return "deep"
    if effort == "low" and mode in {"audit", "seo-geo", "ux"}:
        return "lean"
    return "standard"


def chain_control_block(complexity: str) -> str:
    if complexity == "lean":
        plan = [
            "L0 Scope: 승인 경계와 금지 항목만 확정한다.",
            "L1 Evidence: 확인 가능한 증거만 짧게 수집한다.",
            "L2 Analyze: 증거 기반 finding만 만든다.",
            "L5 Report: 미검증 항목을 분리해 보고한다.",
            "L3/L4는 high-risk, 증거 충돌, 사용자 승인 전 수정안이 있을 때만 실행한다.",
        ]
    elif complexity == "deep":
        plan = [
            "L0 Scope: 범위·승인·롤백·비용/권한 경계를 먼저 고정한다.",
            "L1 Evidence: 입력이 길면 chunk_id/evidence_id로 나누고 raw 근거를 ledger로 남긴다.",
            "L2 Analyze: chunk별 finding을 합치되 evidence_id 없는 주장은 UNKNOWN으로 보낸다.",
            "L3 Hypothesize: 최소 2개 경쟁 가설과 각각을 깨뜨릴 검증을 만든다.",
            "L4 Critique: 자기 감사만으로 닫지 말고 가능한 외부 검증 명령/테스트를 먼저 지정한다.",
            "L1/L2/L3는 L4 verdict가 re-collect/revise이면 최대 2회 반복한다.",
            "L5 Report: 결론, 증거, 미검증, 승인 필요 작업, rollback 계획을 분리한다.",
        ]
    else:
        plan = [
            "L0→L5를 1회 실행한다.",
            "L4에서 evidence 부족이면 L1로, 논리 결함이면 L2/L3로 1회 되돌아간다.",
            "단순 작업은 L3를 짧게 유지하고, high-risk 항목은 L4 외부 검증 후보를 붙인다.",
        ]
    return "## 작업 체인 제어\n" + numbered(plan) + "\n"


def context_state_block(complexity: str) -> str:
    rules = [
        "긴 입력은 한 번에 요약하지 말고 chunk_id, evidence_id, source, confidence를 유지해.",
        "각 주요 단계 끝에 STATE CHECKPOINT를 남겨: current_claims, open_questions, next_verification, approval_needed.",
        "맥락이 부족하면 이전 결론을 확정하지 말고 checkpoint와 evidence ledger 기준으로 재개해.",
    ]
    if complexity == "deep":
        rules.append("소스/로그/문서가 많으면 파일군별 ledger를 만들고 L2에서만 통합해.")
    return "## 컨텍스트 관리 프로토콜\n" + numbered(rules) + "\n"


def external_validation_block(mode: str) -> str:
    gates = [
        "코드/설정 변경 판단: 테스트, 빌드, lint, typecheck, schema validation 중 가장 가까운 검증을 지정해.",
        "웹/서비스 판단: GET 응답, headers, robots/sitemap/llms.txt, auth boundary, browser smoke 중 관련 항목을 지정해.",
        "검증 도구를 실행할 수 없으면 실행 불가 사유와 최소 대체 증거를 미검증 항목에 남겨.",
    ]
    if mode in {"fix", "deploy"}:
        gates.append("승인 전에는 수정/배포하지 말고, 승인 후 rollback 경로와 재검증 명령을 먼저 기록해.")
    return "## 외부 검증 훅\n" + numbered(gates) + "\n"


def external_verdict_block(mode: str, threshold: int = 90) -> str:
    rules = [
        f"L4의 최종 pass/revise 보조 판정은 프롬프트 키워드 린트로만 제공한다. 기본 명령: python3 prompt_ops_maker.py verify --input <prompt.md> --threshold {threshold}.",
        "verify 결과 passed=false 또는 missing 항목 존재 → verdict=revise. 모델은 missing 항목만 보완한다.",
        "이 판정은 프롬프트에 검증/근거/경계 문구가 있는지 확인할 뿐, 테스트 로그나 실행 산출물의 성공을 보증하지 않는다.",
        "실제 완료 판정은 test/build/curl/browser smoke/schema validation 같은 실행 로그를 별도 증거로 확인한 뒤 내린다.",
    ]
    if mode in {"fix", "deploy", "appsec", "deep-audit"}:
        rules.append("high-risk 항목은 테스트/build/curl/schema/browser smoke 중 최소 1개 외부 증거 없이는 완료 처리하지 않는다.")
    return "## 프롬프트 키워드 린트\n" + numbered(rules) + "\n"


def compute_external_verdict(analysis: dict[str, Any], threshold: int) -> dict[str, Any]:
    """Convert prompt keyword lint output into an L4-style helper verdict.

    This is intentionally honest about scope: it proves prompt structure only,
    not that tests, builds, HTTP probes, or browser smokes actually passed.
    """
    missing = [item["id"] for item in analysis.get("missing", [])]
    score = int(analysis.get("score", 0))
    if score >= threshold and not missing:
        verdict = "pass"
        reason = "threshold_met_no_missing_checks"
    elif "verification_gates" in missing or "tool_result_grounding" in missing:
        verdict = "re-collect"
        reason = "evidence_or_verification_gate_missing"
    else:
        verdict = "revise"
        reason = "threshold_or_required_checks_missing"
    return {
        "verdict": verdict,
        "reason": reason,
        "score": score,
        "threshold": threshold,
        "missing": missing,
        "verdict_source": "prompt-keyword-lint",
        "scope": "prompt_text_only_no_runtime_execution_evidence",
    }
...
def render_prompt(
    ...
    *,
    ...
    complexity = infer_complexity_tier(mode=mode, effort=effort, task=task, config=config)
...
"""
    ...
    text += f"\n실행 환경 지침:\n{numbered(env_items)}\n"
    text += f"\n{chain_control_block(complexity)}"
...
검증되지 않은 내부 추론은 결론처럼 쓰지 말고, 결론 / 확인한 증거 / 미검증 항목으로 분리해.
...
def verify(args: argparse.Namespace) -> int:
    ...
    payload = {
        ...
        "analysis": analysis,
        "external_verdict": compute_external_verdict(analysis, args.threshold),
    }
...
def self_verify_block(threshold: int = 90, max_iterations: int = 2) -> str:
    return f"""
## Self-Verification Protocol (Fable 5 — 보조 점검, 최종 판정 아님)

이 블록은 외부 verifier 없이 네가 직접 출력을 검증하는 rubric이다.
완료를 선언하기 전에 아래 루프를 실행해. 최대 {max_iterations}회 재시도.
단, pass/revise 보조 판정은 프롬프트 키워드 린트 결과가 우선한다. 실제 완료 판정에는 별도 실행 로그가 필요하다.
